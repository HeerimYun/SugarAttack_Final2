...
namespace ToucanSystems
{
    ...
    public class SmartChart : MonoBehaviour
    {
        ...
        /// <summary>
        /// Determines if displayed data should be limited by min and max value on X axis.
        /// </summary>
        public bool limitDisplayedData = false;
        /// <summary>
        /// Minimum data range to be displayed.
        /// </summary>
        public float minRange = 0f;
        /// <summary>
        /// Maximum data range to be displayed.
        /// </summary>
...
#pragma warning disable 0414
        ...
        private void OnValidate()
        {
            ...
            {
                if (!dataSet.isDataLineColorSet)
                {
                    dataSet.dataLineColor = Color.black;
                    dataSet.isDataLineColorSet = true;
                }
            }
        }
        ...
        private void DrawData()
        {
            if (chartData != null)
            {
                for (int i = 0; i < chartData.Length; i++)
                {
                    ...
                    float firstXValue = 0;
                    bool isFirstXValueFound = false;

                    for (int j = 0; j < chartData[i].data.Length; j++)
                    {
                        if (!limitDisplayedData || (chartData[i].data[j].x >= (minRange * (maxXValue - minXValue) + minXValue) && chartData[i].data[j].x <= (maxRange * (maxXValue - minXValue) + minXValue)))
                        {
                            if (!isFirstXValueFound)
                            {
                                firstXValue = chartData[i].data[j].x;
                                isFirstXValueFound = true;
                            }
                    ...
                    if (fillAreaUnderLine && finalPoints.Count > 1)
                    {
                        ...
                        if (colorArray.Length == fillTexture.width && colorArray.Length > 2)
                        {
                            ...
                            float shift = (minXValue - firstXValue) / (maxXValue - minXValue);
                            if (minXValue >= finalPoints[0].x)
                            {
                                shift = 0;
                            }
