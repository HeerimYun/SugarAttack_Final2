...
namespace ToucanSystems
{
    ...
    public class SmartChartEditor : Editor
    {
        ...
        private void ChartDataSection()
        {
            ...
            SerializedProperty limitDisplayedData = serializedObject.FindProperty("limitDisplayedData");
            EditorGUILayout.PropertyField(limitDisplayedData, true);
            SerializedProperty minRangeProperty = serializedObject.FindProperty("minRange");
            SerializedProperty maxRangeProperty = serializedObject.FindProperty("maxRange");
            minRange = minRangeProperty.floatValue;
            maxRange = maxRangeProperty.floatValue;
            float fullRange = chart.maxXValue - chart.minXValue;
            if (limitDisplayedData.boolValue)
            {
                EditorGUILayout.LabelField("Displayed Data Range ( " + (minRange * fullRange + chart.minXValue) + " : " + (maxRange * fullRange + chart.minXValue) + " )");
                EditorGUILayout.MinMaxSlider(ref minRange, ref maxRange, 0, 1);
            }
            EditorGUILayout.Space();
